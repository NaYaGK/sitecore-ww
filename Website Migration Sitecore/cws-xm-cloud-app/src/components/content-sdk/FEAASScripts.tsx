...
const FEAASScripts = (): JSX.Element => {
  ...
  const shouldOptimize = (src: string) => {
    if (src.startsWith('http')) {
      ...
      const domains: string[] = (imagesConfig as Record<string, unknown>)?.domains as string[] || [];
      const remotePatterns = imagesConfig?.remotePatterns || [];
      return (
  ...
  FEAAS.setElementImplementation('img', (attributes: Record<string, string>) => {
    ...
    return (
      <Image
        height={1920}
        width={1200}
        unoptimized={!shouldOptimize(imageSrc)}
        src={imageSrc}
        alt={alt || ''}
        {...imgAttributes}
      />
