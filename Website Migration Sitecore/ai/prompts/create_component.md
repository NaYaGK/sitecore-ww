**Variables**
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `{COMPONENT_SPEC_PATH}`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/{COMPONENT_NAME}`. Within that folder include:

  - `{COMPONENT_NAME}.tsx` — main component (strictly typed props, exported).
  - `{COMPONENT_NAME}.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `{component_name}.props.ts` — Sitecore field/interface definitions if needed.
  - `{COMPONENT_NAME}.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `{COMPONENT_NAME}.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `{COMPONENT_NAME}.stories.tsx` must render the component using realistic mock data derived from the spec.
