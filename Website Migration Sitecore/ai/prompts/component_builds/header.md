**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/header.md`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/header.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/Header`. Within that folder include:

  - `Header.tsx` — main component (strictly typed props, exported).
  - `Header.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `Header.props.ts` — Sitecore field/interface definitions if needed.
  - `Header.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `Header.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `Header.stories.tsx` must render the component using realistic mock data derived from the spec.
