**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/accordion.md`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/accordion.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/Accordion`. Within that folder include:

  - `Accordion.tsx` — main component (strictly typed props, exported).
  - `Accordion.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `Accordion.props.ts` — Sitecore field/interface definitions if needed.
  - `Accordion.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `Accordion.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `Accordion.stories.tsx` must render the component using realistic mock data derived from the spec.
