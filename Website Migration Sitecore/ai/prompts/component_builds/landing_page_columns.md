**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/landing_page_columns.md`
    - `{COMPONENT_NAME}` — `LandingPageColumns`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/landing_page_columns.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/LandingPageColumns`. Within that folder include:

  - `LandingPageColumns.tsx` — main component (strictly typed props, exported).
  - `LandingPageColumns.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `LandingPageColumns.props.ts` — Sitecore field/interface definitions if needed.
  - `LandingPageColumns.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `LandingPageColumns.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `LandingPageColumns.stories.tsx` must render the component using realistic mock data derived from the spec.
