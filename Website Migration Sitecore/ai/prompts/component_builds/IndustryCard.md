**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/IndustryCard.md`
    - `{COMPONENT_NAME}` — `IndustryCard`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/IndustryCard.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/IndustryCard`. Within that folder include:

  - `IndustryCard.tsx` — main component (strictly typed props, exported).
  - `IndustryCard.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `IndustryCard.props.ts` — Sitecore field/interface definitions if needed.
  - `IndustryCard.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `IndustryCard.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `IndustryCard.stories.tsx` must render the component using realistic mock data derived from the spec.
