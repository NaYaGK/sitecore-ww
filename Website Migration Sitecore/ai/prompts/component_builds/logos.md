**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/logos.md`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/logos.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/Logos`. Within that folder include:

  - `Logos.tsx` — main component (strictly typed props, exported).
  - `Logos.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `Logos.props.ts` — Sitecore field/interface definitions if needed.
  - `Logos.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `Logos.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `Logos.stories.tsx` must render the component using realistic mock data derived from the spec.
