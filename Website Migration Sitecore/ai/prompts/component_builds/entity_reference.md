**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/entity_reference.md`
    - `{COMPONENT_NAME}` — `EntityReference`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/entity_reference.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/EntityReference`. Within that folder include:

  - `EntityReference.tsx` — main component (strictly typed props, exported).
  - `EntityReference.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `EntityReference.props.ts` — Sitecore field/interface definitions if needed.
  - `EntityReference.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `EntityReference.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `EntityReference.stories.tsx` must render the component using realistic mock data derived from the spec.
