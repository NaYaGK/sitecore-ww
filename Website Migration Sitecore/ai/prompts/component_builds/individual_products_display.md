**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/individual_products_display.md`
    - `{COMPONENT_NAME}` — `IndividualProductsDisplay`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/individual_products_display.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/IndividualProductsDisplay`. Within that folder include:

  - `IndividualProductsDisplay.tsx` — main component (strictly typed props, exported).
  - `IndividualProductsDisplay.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `IndividualProductsDisplay.props.ts` — Sitecore field/interface definitions if needed.
  - `IndividualProductsDisplay.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `IndividualProductsDisplay.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `IndividualProductsDisplay.stories.tsx` must render the component using realistic mock data derived from the spec.
