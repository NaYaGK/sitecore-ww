**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/simple_heading.md`
    - `{COMPONENT_NAME}` — `SimpleHeading`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/simple_heading.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/SimpleHeading`. Within that folder include:

  - `SimpleHeading.tsx` — main component (strictly typed props, exported).
  - `SimpleHeading.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `SimpleHeading.props.ts` — Sitecore field/interface definitions if needed.
  - `SimpleHeading.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `SimpleHeading.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `SimpleHeading.stories.tsx` must render the component using realistic mock data derived from the spec.
