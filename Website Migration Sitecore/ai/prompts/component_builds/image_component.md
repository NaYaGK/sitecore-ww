**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/image_component.md`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/image_component.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/ImageComponent`. Within that folder include:

  - `ImageComponent.tsx` — main component (strictly typed props, exported).
  - `ImageComponent.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `ImageComponent.props.ts` — Sitecore field/interface definitions if needed.
  - `ImageComponent.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `ImageComponent.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `ImageComponent.stories.tsx` must render the component using realistic mock data derived from the spec.
