**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/areas_component.md`
    - `{COMPONENT_NAME}` — `AreasComponent`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/areas_component.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/AreasComponent`. Within that folder include:

  - `AreasComponent.tsx` — main component (strictly typed props, exported).
  - `AreasComponent.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `AreasComponent.props.ts` — Sitecore field/interface definitions if needed.
  - `AreasComponent.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `AreasComponent.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `AreasComponent.stories.tsx` must render the component using realistic mock data derived from the spec.
