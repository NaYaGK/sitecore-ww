**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/wysiwyg_block.md`
    - `{COMPONENT_NAME}` — `WysiwygBlock`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/wysiwyg_block.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/WysiwygBlock`. Within that folder include:

  - `WysiwygBlock.tsx` — main component (strictly typed props, exported).
  - `WysiwygBlock.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `WysiwygBlock.props.ts` — Sitecore field/interface definitions if needed.
  - `WysiwygBlock.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `WysiwygBlock.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `WysiwygBlock.stories.tsx` must render the component using realistic mock data derived from the spec.
