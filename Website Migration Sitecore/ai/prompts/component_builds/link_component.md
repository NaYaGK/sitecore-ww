**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/link_component.md`
    - `{COMPONENT_NAME}` — `LinkComponent`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/link_component.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/LinkComponent`. Within that folder include:

  - `LinkComponent.tsx` — main component (strictly typed props, exported).
  - `LinkComponent.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `LinkComponent.props.ts` — Sitecore field/interface definitions if needed.
  - `LinkComponent.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `LinkComponent.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `LinkComponent.stories.tsx` must render the component using realistic mock data derived from the spec.
