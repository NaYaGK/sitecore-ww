**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/video_player_component.md`
    - `{COMPONENT_NAME}` — `VideoPlayerComponent`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/video_player_component.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/VideoPlayerComponent`. Within that folder include:

  - `VideoPlayerComponent.tsx` — main component (strictly typed props, exported).
  - `VideoPlayerComponent.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `VideoPlayerComponent.props.ts` — Sitecore field/interface definitions if needed.
  - `VideoPlayerComponent.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `VideoPlayerComponent.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `VideoPlayerComponent.stories.tsx` must render the component using realistic mock data derived from the spec.
