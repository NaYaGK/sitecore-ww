**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/wysiwyg_table.md`
    - `{COMPONENT_NAME}` — `WysiwygTable`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/wysiwyg_table.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/WysiwygTable`. Within that folder include:

  - `WysiwygTable.tsx` — main component (strictly typed props, exported).
  - `WysiwygTable.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `WysiwygTable.props.ts` — Sitecore field/interface definitions if needed.
  - `WysiwygTable.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `WysiwygTable.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `WysiwygTable.stories.tsx` must render the component using realistic mock data derived from the spec.
