**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/text_highlight.md`
    - `{COMPONENT_NAME}` — `TextHighlight`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/text_highlight.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/TextHighlight`. Within that folder include:

  - `TextHighlight.tsx` — main component (strictly typed props, exported).
  - `TextHighlight.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `TextHighlight.props.ts` — Sitecore field/interface definitions if needed.
  - `TextHighlight.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `TextHighlight.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `TextHighlight.stories.tsx` must render the component using realistic mock data derived from the spec.
