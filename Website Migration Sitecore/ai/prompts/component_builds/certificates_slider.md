**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/certificates_slider.md`
    - `{COMPONENT_NAME}` — `CertificatesSlider`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/certificates_slider.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/CertificatesSlider`. Within that folder include:

  - `CertificatesSlider.tsx` — main component (strictly typed props, exported).
  - `CertificatesSlider.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `CertificatesSlider.props.ts` — Sitecore field/interface definitions if needed.
  - `CertificatesSlider.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `CertificatesSlider.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `CertificatesSlider.stories.tsx` must render the component using realistic mock data derived from the spec.
