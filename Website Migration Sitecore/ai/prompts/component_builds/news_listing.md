**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/news_listing.md`
    - `{COMPONENT_NAME}` — `NewsListing`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/news_listing.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/NewsListing`. Within that folder include:

  - `NewsListing.tsx` — main component (strictly typed props, exported).
  - `NewsListing.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `NewsListing.props.ts` — Sitecore field/interface definitions if needed.
  - `NewsListing.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `NewsListing.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `NewsListing.stories.tsx` must render the component using realistic mock data derived from the spec.
