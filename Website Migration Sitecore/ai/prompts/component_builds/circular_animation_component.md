**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/circular_animation_component.md`
    - `{COMPONENT_NAME}` — `CircularAnimationComponent`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/circular_animation_component.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/CircularAnimationComponent`. Within that folder include:

  - `CircularAnimationComponent.tsx` — main component (strictly typed props, exported).
  - `CircularAnimationComponent.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `CircularAnimationComponent.props.ts` — Sitecore field/interface definitions if needed.
  - `CircularAnimationComponent.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `CircularAnimationComponent.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `CircularAnimationComponent.stories.tsx` must render the component using realistic mock data derived from the spec.
