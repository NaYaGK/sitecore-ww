**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/hero_banner.md`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/hero_banner.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/HeroBanner`. Within that folder include:

  - `HeroBanner.tsx` — main component (strictly typed props, exported).
  - `HeroBanner.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `HeroBanner.props.ts` — Sitecore field/interface definitions if needed.
  - `HeroBanner.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `HeroBanner.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `HeroBanner.stories.tsx` must render the component using realistic mock data derived from the spec.
