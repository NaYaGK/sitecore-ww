**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/breadcrumb_navigation.md`
    - `{COMPONENT_NAME}` — `BreadcrumbNavigation`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/breadcrumb_navigation.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/BreadcrumbNavigation`. Within that folder include:

  - `BreadcrumbNavigation.tsx` — main component (strictly typed props, exported).
  - `BreadcrumbNavigation.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `BreadcrumbNavigation.props.ts` — Sitecore field/interface definitions if needed.
  - `BreadcrumbNavigation.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `BreadcrumbNavigation.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `BreadcrumbNavigation.stories.tsx` must render the component using realistic mock data derived from the spec.
