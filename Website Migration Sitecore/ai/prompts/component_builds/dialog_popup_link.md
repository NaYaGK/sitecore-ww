**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/dialog_popup_link.md`
    - `{COMPONENT_NAME}` — `DialogPopupLink`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/dialog_popup_link.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/DialogPopupLink`. Within that folder include:

  - `DialogPopupLink.tsx` — main component (strictly typed props, exported).
  - `DialogPopupLink.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `DialogPopupLink.props.ts` — Sitecore field/interface definitions if needed.
  - `DialogPopupLink.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `DialogPopupLink.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `DialogPopupLink.stories.tsx` must render the component using realistic mock data derived from the spec.
