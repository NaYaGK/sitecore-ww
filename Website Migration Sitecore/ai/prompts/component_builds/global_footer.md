**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/global_footer.md`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/global_footer.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/GlobalFooter`. Within that folder include:

  - `GlobalFooter.tsx` — main component (strictly typed props, exported).
  - `GlobalFooter.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `GlobalFooter.props.ts` — Sitecore field/interface definitions if needed.
  - `GlobalFooter.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `GlobalFooter.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `GlobalFooter.stories.tsx` must render the component using realistic mock data derived from the spec.
