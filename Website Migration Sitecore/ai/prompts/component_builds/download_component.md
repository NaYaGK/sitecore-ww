**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/download_component.md`
    - `{COMPONENT_NAME}` — `DownloadComponent`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/download_component.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/DownloadComponent`. Within that folder include:

  - `DownloadComponent.tsx` — main component (strictly typed props, exported).
  - `DownloadComponent.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `DownloadComponent.props.ts` — Sitecore field/interface definitions if needed.
  - `DownloadComponent.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `DownloadComponent.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `DownloadComponent.stories.tsx` must render the component using realistic mock data derived from the spec.
