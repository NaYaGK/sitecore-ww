**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/image_gallery_product_list.md`
    - `{COMPONENT_NAME}` — `ImageGalleryProductList`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/image_gallery_product_list.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/ImageGalleryProductList`. Within that folder include:

  - `ImageGalleryProductList.tsx` — main component (strictly typed props, exported).
  - `ImageGalleryProductList.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `ImageGalleryProductList.props.ts` — Sitecore field/interface definitions if needed.
  - `ImageGalleryProductList.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `ImageGalleryProductList.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `ImageGalleryProductList.stories.tsx` must render the component using realistic mock data derived from the spec.
