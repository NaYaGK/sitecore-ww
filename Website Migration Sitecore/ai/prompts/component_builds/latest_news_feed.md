**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/latest_news_feed.md`
    - `{COMPONENT_NAME}` — `LatestNewsFeed`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/latest_news_feed.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/LatestNewsFeed`. Within that folder include:

  - `LatestNewsFeed.tsx` — main component (strictly typed props, exported).
  - `LatestNewsFeed.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `LatestNewsFeed.props.ts` — Sitecore field/interface definitions if needed.
  - `LatestNewsFeed.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `LatestNewsFeed.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `LatestNewsFeed.stories.tsx` must render the component using realistic mock data derived from the spec.
