**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/sticky_contact_bar.md`
    - `{COMPONENT_NAME}` — `StickyContactBar`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/sticky_contact_bar.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/StickyContactBar`. Within that folder include:

  - `StickyContactBar.tsx` — main component (strictly typed props, exported).
  - `StickyContactBar.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `StickyContactBar.props.ts` — Sitecore field/interface definitions if needed.
  - `StickyContactBar.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `StickyContactBar.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `StickyContactBar.stories.tsx` must render the component using realistic mock data derived from the spec.
