**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/advantage_card.md`
    - `{COMPONENT_NAME}` — `AdvantageCard`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/advantage_card.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/AdvantageCard`. Within that folder include:

  - `AdvantageCard.tsx` — main component (strictly typed props, exported).
  - `AdvantageCard.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `AdvantageCard.props.ts` — Sitecore field/interface definitions if needed.
  - `AdvantageCard.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `AdvantageCard.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `AdvantageCard.stories.tsx` must render the component using realistic mock data derived from the spec.
