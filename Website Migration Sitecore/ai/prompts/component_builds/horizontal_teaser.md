**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/horizontal_teaser.md`
    - `{COMPONENT_NAME}` — `HorizontalTeaser`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/horizontal_teaser.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/HorizontalTeaser`. Within that folder include:

  - `HorizontalTeaser.tsx` — main component (strictly typed props, exported).
  - `HorizontalTeaser.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `HorizontalTeaser.props.ts` — Sitecore field/interface definitions if needed.
  - `HorizontalTeaser.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `HorizontalTeaser.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `HorizontalTeaser.stories.tsx` must render the component using realistic mock data derived from the spec.
