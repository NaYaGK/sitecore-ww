**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/contact_form_banner.md`
    - `{COMPONENT_NAME}` — `ContactFormBanner`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/contact_form_banner.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/ContactFormBanner`. Within that folder include:

  - `ContactFormBanner.tsx` — main component (strictly typed props, exported).
  - `ContactFormBanner.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `ContactFormBanner.props.ts` — Sitecore field/interface definitions if needed.
  - `ContactFormBanner.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `ContactFormBanner.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `ContactFormBanner.stories.tsx` must render the component using realistic mock data derived from the spec.
