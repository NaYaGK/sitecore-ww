**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/slider_component.md`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/slider_component.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/SliderComponent`. Within that folder include:

  - `SliderComponent.tsx` — main component (strictly typed props, exported).
  - `SliderComponent.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `SliderComponent.props.ts` — Sitecore field/interface definitions if needed.
  - `SliderComponent.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `SliderComponent.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `SliderComponent.stories.tsx` must render the component using realistic mock data derived from the spec.
