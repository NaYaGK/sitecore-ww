**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/footer.md`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/footer.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/Footer`. Within that folder include:

  - `Footer.tsx` — main component (strictly typed props, exported).
  - `Footer.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `Footer.props.ts` — Sitecore field/interface definitions if needed.
  - `Footer.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `Footer.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `Footer.stories.tsx` must render the component using realistic mock data derived from the spec.
