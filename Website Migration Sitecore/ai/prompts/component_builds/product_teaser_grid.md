**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/product_teaser_grid.md`
    - `{COMPONENT_NAME}` — `ProductTeaserGrid`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/product_teaser_grid.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/ProductTeaserGrid`. Within that folder include:

  - `ProductTeaserGrid.tsx` — main component (strictly typed props, exported).
  - `ProductTeaserGrid.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `ProductTeaserGrid.props.ts` — Sitecore field/interface definitions if needed.
  - `ProductTeaserGrid.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `ProductTeaserGrid.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `ProductTeaserGrid.stories.tsx` must render the component using realistic mock data derived from the spec.
