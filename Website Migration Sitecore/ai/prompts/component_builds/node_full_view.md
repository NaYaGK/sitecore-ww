**Variables**
    - `{COMPONENT_SPEC_PATH}` — `ai/prompts/component_specs/node_full_view.md`
    - `{COMPONENT_NAME}` — `NodeFullView`
  ...
  **1. Understand the spec and reference UI**
  ...
  - Read the component requirements at `ai/prompts/component_specs/node_full_view.md`.
  ...
  **2. Plan the files and structure**
  ...
  The component must live under `src/components/NodeFullView`. Within that folder include:

  - `NodeFullView.tsx` — main component (strictly typed props, exported).
  - `NodeFullView.module.scss` — all styling here; use Sass features (variables, nesting, mixins) as needed.
  - `NodeFullView.props.ts` — Sitecore field/interface definitions if needed.
  - `NodeFullView.stories.tsx` — Storybook story with realistic mock data.
  ...
  **3. Implementation specifics**
  ...
  - Mirror the reference design exactly in `NodeFullView.module.scss`: fonts, sizes, weights, spacing, border radii, shadows, responsive layout, motion.
  ...
  **4. Storybook-first deliverable**
  ...
  - `NodeFullView.stories.tsx` must render the component using realistic mock data derived from the spec.
